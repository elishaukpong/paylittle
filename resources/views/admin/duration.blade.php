...
<div class="jumbotron text-center bg-primary duration-jumbotron-cover">
    <div class="jumbotron-cover py-4">
        <h1 class="font-weight-bold text-white ">Duration</h1>
    </div>
</div>
                    ...
                    <form action="{{route('duration.store')}}" METHOD="POST">
                        ...
                        <div class="form-group py-2">
                            <input id="timeline" type="text" class="form-control {{ $errors->has('timeline') ? ' is-invalid' : '' }}" name="timeline"
                                 required autofocus placeholder="Enter Number of Months"> @if ($errors->has('timeline'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('timeline') }}</strong>
                            </span> @endif
                        </div>

                        <input type="submit" value="Create Duration" class="btn btn-primary form-control">
                    </form>
                </div>
            </div>
        </div>
